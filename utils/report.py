...
def generate_pdf_report(data, filename="eco_report.pdf"):
    ...
    if not history.empty:
        ...
        history["co2"].cumsum().plot(marker="o")
        plt.title("Cumulative CO₂ Impact")
        plt.xlabel("Scan Number")
        plt.ylabel("CO₂ (kg)")
        plt.savefig(trend_file, bbox_inches="tight")
        plt.close()
    ...
    elements.append(Paragraph("EcoSort AI - Environmental Impact Report", styles["Title"]))
    ...
    # ---------------- BREAKDOWN TABLE ----------------
    waste_table_data = [["Waste Type", "Count"]]

    for waste, count in breakdown.items():
        waste_table_data.append([waste.capitalize(), str(count)])
    ...
    elements.append(Paragraph("Waste Breakdown", styles["Heading2"]))
    elements.append(waste_table)
    elements.append(Spacer(1, 20))

    # ---------------- PIE CHART ----------------
    elements.append(Paragraph("Waste Distribution", styles["Heading2"]))
    elements.append(Image(pie_file, width=300, height=300))
    elements.append(Spacer(1, 20))

    # ---------------- TREND ----------------
    if not history.empty:
        elements.append(Paragraph("CO₂ Trend", styles["Heading2"]))
        elements.append(Image(trend_file, width=400, height=250))
        elements.append(Spacer(1, 20))
